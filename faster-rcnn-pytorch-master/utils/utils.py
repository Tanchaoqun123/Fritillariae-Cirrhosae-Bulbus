...
class DecodeBox():
    ...
    def forward(self, roi_cls_locs, roi_scores, rois ,feature,height, width, score_thresh):
        ...
        for l in range(1, self.num_classes):
            ...
            detections_class = np.concatenate(
                [cls_bbox_l, np.expand_dims(prob_l, axis=-1), np.expand_dims(label, axis=-1), featu], axis=-1)
